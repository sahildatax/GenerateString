...
async def main(_, msg):
    await msg.reply(
        "Choose Which String You Want To Generate",
...
async def generate_session(bot, msg, telethon=False):
    await msg.reply("Starting {} Session Generator...".format("Telethon" if telethon else "Pyrogram"))
    user_id = msg.chat.id
    api_id_msg = await bot.ask(user_id, 'Enter  `API_ID`', filters=filters.text)
    ...
    try:
        ...
        return
    api_hash_msg = await bot.ask(user_id, 'Enter `API_HASH`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    api_hash = api_hash_msg.text
    phone_number_msg = await bot.ask(user_id, 'Enter `PHONE_NUMBER` with country code. \nExample : `+628xxxxxxx`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nSession Generated".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Successfully Generated {} string session. \n\nCheck Saved Messages!".format("telethon" if telethon else "pyrogram"))
...
#     if "/cancel" in msg.text:
#         await msg.reply("Cancel All Processes!", quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
#         return True
#     elif "/restart" in msg.text:
#         await msg.reply("Restart Bot!", quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
#         return True
